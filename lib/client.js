window.__ModuleLoader__.load({ id: "dsh-markdown-link-preview", factory(require) { var module = { exports: {} }; var exports = module.exports;
...
// src/core.js
var MARKDOWN_NAME = /\.md(?:own)?(?:$|[?#])/iu;
function isMarkdownOutputLink(anchor, locationHref) {
...
function extractMarkdownPath(text2) {
  const normalized = String(text2 ?? "").replace(/\\/gu, "/").trim();
...
function isSafePreviewUrl(href, locationHref) {
  try {
...
function markdownTabSeed(url, title) {
...
function displayName(anchor, href, locationHref) {
...
var TAB_ID = "markdown-link-preview";
var styles = `
...
function renderMarkdown(source, baseUrl) {
  const html3 = marked.parse(source, { async: false, gfm: true, breaks: false });
...
function MarkdownLinkPreview({ tab, visible }) {
  ...
  const [state, setState] = (0, import_react.useState)({ loading: true, source: "", error: "" });
  (0, import_react.useEffect)(() => {
    if (!visible || typeof url !== "string") return void 0;
    ...
    }).then((source) => setState({ loading: false, source, error: "" })).catch((error) => {
      if (error.name !== "AbortError") setState({ loading: false, source: "", error: error.message || "Unable to load Markdown" });
    });
    return () => controller.abort();
  }, [url, visible]);
  const html3 = (0, import_react.useMemo)(() => state.source ? renderMarkdown(state.source, url) : "", [state.source, url]);
  return import_react.default.createElement(
    "article",
    { className: "dsh-md-link-preview" },
    import_react.default.createElement("h1", { className: "dsh-md-link-preview__title" }, tab.title),
    state.loading && import_react.default.createElement("p", { className: "dsh-md-link-preview__status" }, "\u6B63\u5728\u52A0\u8F7D Markdown\u2026"),
    state.error && import_react.default.createElement("p", { className: "dsh-md-link-preview__error" }, `\u65E0\u6CD5\u9884\u89C8\uFF1A${state.error}`),
    !state.loading && !state.error && import_react.default.createElement("div", {
...
function apply2(ctx) {
  ...
  ctx.effect(() => {
    const onClick = (event) => {
      ...
      const source = event.target instanceof Element ? event.target.closest('a[href], button, [role="button"], [data-file-path]') : null;
